...
# $Id: TC_Follow.pm,v 1.1 2005/11/14 16:35:45 guido Exp $
...
sub testAddress {
    ...
    my $data = {};
    bless $data, 'Data::Traverse::Fake';

    my $wanted = sub {
	my $address = int $_;
	$self->assert_equals ($address, $Data::Traverse::address);
    };
    traverse { wanted => $wanted }, $data;
}
...
sub testSeen {
	...
	unless ('ARRAY' eq ref $_) {
	    $self->assert_equals ($count++, $Data::Traverse::seen);
	}
    };
    traverse { wanted => $wanted }, $data;
